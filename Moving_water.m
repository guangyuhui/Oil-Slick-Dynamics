...
for m=1:1000
    ...
    dt=min(0.2*dx/(max(abs(q))+1));%,dx^2/4);
...
%     Sigma2(1,:)=sigma2;
    ...
    t=t+dt;
     ...
     for j=1:length(x)
         ...
         B=I_uj*u(:,j)+0.25*niu*dt*D/dy^2*u(:,j)-0.25*niu*dt/dy*Sigma1(:,j)-dt/(4*dy)*h(j)*[d_q_sq(j);zeros(length(y)-1,1)];
         u(:,j)=A\B;
     end

     u=u-(0.5*dt/dx)*u.*(u*d2);%+(dt/dx)*u_free.*(u_free*d2);
